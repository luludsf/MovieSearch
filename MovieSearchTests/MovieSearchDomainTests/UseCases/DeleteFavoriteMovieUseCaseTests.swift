//
//  DeleteFavoriteMovieUseCaseTests.swift
...
final class DeleteFavoriteMovieUseCaseTests: XCTestCase {
    
    var sut: DeleteFavoriteMovieUseCase!
    var mockRepository: MockFavoriteMoviesRepository!
    
    override func setUp() {
        super.setUp()
        mockRepository = MockFavoriteMoviesRepository()
        sut = DeleteFavoriteMovieUseCase(repository: mockRepository)
    }
    ...
    func test_execute_WhenSuccessful_ShouldReturnTrue() {
        // Given
        let expectation = XCTestExpectation(description: "Successo ao deletar")
        let movie = Movie(
            id: 198884,
        ...
        sut.execute(movie) { success in
            ...
            XCTAssertEqual(self.mockRepository.deletedMovies.count, 1)
            XCTAssertEqual(self.mockRepository.deletedMovies.first?.id, 198884)
            expectation.fulfill()
        }
    ...
    func test_execute_WhenFailure_ShouldReturnFalse() {
        // Given
        let expectation = XCTestExpectation(description: "Erro ao deletar o filme")
        let movie = Movie(
            id: 198884,
            originalTitle: "Barbie and The Sensations: Rockin' Back to Earth",
            posterPath: "/vUCqvymxUwYxp9H6jw5R5UiaeE5.jpg",
        ...
        sut.execute(movie) { success in
            ...
            XCTAssertEqual(self.mockRepository.deletedMovies.count, 0)
            expectation.fulfill()
        }
